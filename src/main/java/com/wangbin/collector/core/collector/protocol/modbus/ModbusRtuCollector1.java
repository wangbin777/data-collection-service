...
@Slf4j
@Component
public class ModbusRtuCollector1 extends BaseCollector {
    ...
    private Map<String, Object> batchReadGroup(RegisterType type, List<GroupedPoint> pointGroup) throws Exception {
        ...
        int minAddress = pointGroup.stream()
                .mapToInt(gp -> gp.address.getAddress())
                .min()
                .orElse(0);
    ...
    private Map<String, Object> batchReadCoils(List<GroupedPoint> pointGroup, int minAddress, int quantity) throws Exception {
        ...
        try {
            ...
            if (response != null && response.coils() != null) {
                byte[] raw = response.coils();
                int bytes = raw.length;

                for (GroupedPoint gp : pointGroup) {
                    int offset = gp.address.getAddress() - minAddress;
    ...
    private Map<String, Object> batchReadHoldingRegisters(List<GroupedPoint> pointGroup, int minAddress, int quantity) throws Exception {
        ...
        int maxRegisters = pointGroup.stream()
                .mapToInt(gp -> DataType.fromString(gp.point.getDataType()).getRegisterCount())
                .max()
                .orElse(1);
        ...
        try {
            ...
            if (response != null && response.registers() != null) {
                ...
                for (GroupedPoint gp : pointGroup) {
                    int offsetRegister = gp.address.getAddress() - minAddress;
                    int registersNeeded = DataType.fromString(gp.point.getDataType()).getRegisterCount();
                    ...
                    if ((offsetRegister + registersNeeded) * 2 <= raw.length) {
                        ...
                        Object value = ModbusUtils.parseValue(registerData, 0, gp.point.getDataType());
                        results.put(gp.point.getPointId(), value);
                    }
                }
            }
        ...
        try {
            ...
            if (response != null && response.coils() != null) {
                ...
                for (int i = 0; i < quantity; i++) {
                    int byteIndex = i / 8;
                    int bitIndex = i % 8;
                    boolean value = byteIndex < byteCount && ((raw[byteIndex] >> bitIndex) & 0x01) == 1;
                    values.add(value);
                }
            }
    ...
    private Object executeReadExceptionStatus(Map<String, Object> params) throws Exception {
        try {
            ...
            requestData[1] = 0x07; // 功能码
            byte[] crc = Crc16.calculate(requestData, 2);
            requestData[2] = crc[0];
            requestData[3] = crc[1];

            // 发送请求并接收响应
            // 注意：库可能不直接支持此功能码，这里只是示例
            return Map.of(
                    "success", false,
                    "message", "该功能码可能需要直接串口通信实现"
            );
        } catch (Exception e) {
    ...
    private Object executeDiagnostics(Map<String, Object> params) throws Exception {
        int subFunction = (int) params.getOrDefault("subFunction", 0);
        int data = (int) params.getOrDefault("data", 0x0000);

        // 诊断功能码 0x08
        byte[] requestData = new byte[6];
        requestData[0] = (byte) slaveId;
        requestData[1] = 0x08; // 功能码
        ...
        byte[] crc = Crc16.calculate(requestData, 6);

        // 这里需要扩展库功能或使用底层串口通信
        return Map.of(
                "success", false,
                "message", "诊断功能需要底层串口通信实现"
        );
    }
    ...
    private List<List<GroupedPoint>> groupContinuousPoints(List<GroupedPoint> points) {
        ...
        points.sort(Comparator.comparingInt(o -> o.address.getAddress()));
        ...
        for (GroupedPoint gp : points) {
            int addr = gp.address.getAddress();
